...
import { Search, Paperclip, MessageCircle, ArrowLeft, MoreVertical } from 'lucide-react';
...
function SubcontractorMessages() {
    ...
    const sendFileOverSocket = (media, receiver_id) => {
        const { file } = media;
        const CHUNK_SIZE = 1024 * 1024;
        let offset = 0;
    ...
    useEffect(() => {
        if (!job || jobProcessed || isLoadingConversations) return;

        const jobUserId = job.userId;

        setConversations((prev) => {
            const exists = prev.some(c => c.id === jobUserId);
            ...
            return [newConversation, ...prev.map(c => ({ ...c, active: false }))];
        });

        setTimeout(() => {
            handleConversationClick(jobUserId);
        }, 200);

        setJobProcessed(true);
    }, [job, isLoadingConversations]);
    ...
    useEffect(() => {
        ...
        socket.on("get_main_chat", (chatList) => {
            ...
            setConversations((prev) => {
                if (!formatted.length) {
                    return prev.length ? prev : prev;
                }

                const merged = [...formatted];
                prev.forEach((localConv) => {
                    if (!merged.find((f) => f.id === localConv.id)) {
    ...
    useEffect(() => {
        if (!jobProcessed || isLoadingConversations) return;

        setConversations((prev) => {
            if (!prev.some((c) => c.id === job?.userId)) {
                const newConv = {
                    id: job.userId,
                    name: job.name,
                    avatar: job.image || "",
                                ...
                                <div className="flex-1 bg-[#F6F6F7] overflow-y-auto no-scrollbar">
                                    {isLoadingConversations ? (
                                        ...
                                        conversations.map((conversation) => (
                                            <div
                                                key={conversation.id}
                                                className={`flex items-center p-3 sm:p-4 cursor-pointer transition-colors border-b border-[#ebebeb] ${selectedConversation === conversation.id
                                                    ? "bg-[#152A45] text-white mx-2 sm:mx-0 rounded-[12px] sm:rounded-[16px] mb-1"
                                                    : "hover:bg-gray-50"
                                                    }`}
